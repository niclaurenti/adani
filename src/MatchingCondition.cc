...
Value MatchingCondition::MuIndependentTerm(double x, int nf) const {
    return MuIndependentNfIndependentTerm(x)
           + nf * MuIndependentNfDependentTerm(x);
}
...
//  matching condition Qg at O(as^3). Exact result.
//------------------------------------------------------------------------------------------//
...
//==========================================================================================//
//  nf-independent part of the mu-independent part of the unrenormalized
//  matching condition Qg at O(as^3). Approximation from Giacomo Magni, based on
//  the results of [arXiv:2403.00513].
//------------------------------------------------------------------------------------------//
...
array<double, 3> MatchingCondition::a_Qg_30_gm(double x) const {
    ...
    double res = -7685.812499211437 + 8956.649545 / x - 18891.90044109861 * x
                 ...
                 + 4.380199906 * L13 - 8.20987654 * L14
                 + 3.7037037039999996 * L15 + 10739.21741 * L
                 + 1548.8916669999999 * L / x - 7861.809052567688 * x * L
                 - 720.0483828 * L2 + 514.0912722 * L3 - 21.75925926 * L4
                 + 4.844444444 * L5;
...
//==========================================================================================//
//  nf-independent part of the mu-independent part of the unrenormalized
//  matching condition Qg at O(as^3). Approximation from Eq. (3.49) of Ref.
//  [arXiv:1205.5727].
//------------------------------------------------------------------------------------------//
...
double MatchingCondition::a_Qg_30_klmv_up(double x) const {
    ...
    return (
        354.1002 * L13 + 479.3838 * L12 - 7856.784 * (2. - x) - 6233.530 * L2
        + 9416.621 / x + 1548.891 / x * L
    );
}

//==========================================================================================//
//  nf-independent part of the mu-independent part of the unrenormalized
//  matching condition Qg at O(as^3). Approximation from Eq. (15-16) Ref. of
//  [arXiv:1701.05838]
//------------------------------------------------------------------------------------------//
...
array<double, 3> MatchingCondition::a_Qg_30_abmp(double x) const {
    ...
    double lower = 226.3840 * L13 - 652.2045 * L12 - 2686.387 * L1
...
//==========================================================================================//
//  nf-independent part of the mu-independent part of the unrenormalized
//  matching condition Qg at O(as^3). Approximation from Eq. (3.49-50) of Ref.
//  [arXiv:1205.5727].
//------------------------------------------------------------------------------------------//
...
array<double, 3> MatchingCondition::a_Qg_30_klmv(double x) const {
    ...
    double lower = -2658.323 * L12 - 7449.948 * L1 - 7460.002 * (2. - x)
...
array<double, 3> MatchingCondition::a_Qq_PS_30_exact(double x) const {
    ...
    const double H01m1 = Hr3[HPL_map(0, 1, -1)];
    const double H0m11 = Hr3[HPL_map(0, -1, 1)];
    ...
    const double H000m1 = Hr4[HPL_map(0, 0, 0, -1)];
    ...
    const double H0000m1 = Hr5[HPL_map(0, 0, 0, 0, -1)];
    ...
    const double H00001 = Hr5[HPL_map(0, 0, 0, 0, 1)];
    ...
    const double H01111 = Hr5[HPL_map(0, 1, 1, 1, 1)];
    ...
    double B_4 =
        -4. * zeta2 * ln2_2 + 2. / 3 * ln2_4 - 13. / 2 * zeta4 + 16 * Li_412;
        ...
        (+CF * TR * TR
             * (-32. / 81 * (x - 1.) / x * (4. * x2 + 7. * x + 4.) * H1_3
                ...
                + (-32. / 9 * (x - 1) / x * (4 * x2 + 7 * x + 4) * H0
                   - 32. / 405 * (x - 1) / x
                         * (12 * x5 - 108 * x4 - 48 * x3 + 172 * x2 - 2183 * x
                            - 200))
                      * H1_2
               ...
               * (2. / 27 * (x - 1) / x * (4 * x2 + 7 * x + 4) * H1_4
                  ...
                  + (-32. / 3 * 1. / x * (8 * x3 - 30 * x2 - 15 * x - 2)
                     + 128 * (x + 1) * H0)
                        * B_4
                  + 256 * (x + 1) * B_4 * ln2
                  + 944. / 3 * (x + 1) * zeta2 * zeta3 - 3544 * (x + 1) * zeta5
                  + 4096 * (x + 1) * Li_512 - 512. / 15 * ln2_5 * (x + 1))
         ...
         + CA * CF * TR
               * (-2. / 27 * (x - 1) / x * (4 * x2 + 7 * x + 4) * H1_4
                  ...
                  + (-112. / 27 * (x + 1) / x * (4 * x2 - 7 * x + 4) * Hm1 * Hm1
                         * Hm1
                     + 32. / 27 * (x + 1) / x * (188 * x2 - 83 * x + 80) * Hm1
                           * Hm1
                  ...
                  + (4. / 27 * (x - 1) / x * (152 * x2 + 203 * x + 152) * H0_3
                     ...
                     + 8. / 81 * (x - 1) / x * (21512 * x2 - 1057 * x + 7436)
                           * H0
                     - 4. / 9 * 1. / x * (936 * x3 - 640 * x2 + 379 * x - 684)
                           * H0_2)
                        * H1
                  ...
                  + (-8. / 9 * (x + 1) / x * (32 * x2 + 61 * x + 32) * Hm1 * Hm1
                     - 16. / 27 * 1. / x * (82 * x3 - 78 * x2 - 267 * x - 80)
                           * Hm1
                     - 4. / 9 * 1. / x * (144 * x3 - 555 * x2 - 471 * x - 344)
                           * H0_2
                     - 8. / 81 * 1. / (x + 1) / x
                  ...
                  + (-8. / 3 * (x + 1) / x * (88 * x2 - 169 * x + 88) * Hm1
                     ...
                     + 8. / 9 * 1. / x * (136 * x3 - 1605 * x2 - 591 * x - 536)
                           * H0
                     + 8. / 27 * 1. / x
                  ...
                  + (-176. / 9 * (x - 1) / x * (4 * x2 + 7 * x + 4) * H1
                     - 16. / 9 * 1. / x * (148 * x3 + 189 * x2 - 45 * x - 140)
                           * H0
                     - 8. / 27 * 1. / x
    ...
    double tildeaQqPS30 =
         ...
         * (-64 * (x + 1) * H0 * H0 * tildeH0m1m1
            ...
            + 64 * (x + 1) * (tildeH0m1m1 - tildeH0m11 + tildeH01m1 - tildeH011)
                  * zeta2
            ...
            + (-128. / 3 * (x - 1) / x * (4 * x2 + 7 * x + 4) * H1 * tildeH0m1
               ...
               + (-128. / 3 * (x - 1) / x * (4 * x2 + 7 * x + 4) * H1
                  - 32. / 3 * 1. / x * (32 * x3 + 200 * x2 - 104 * x + 1)
                  - 128. / 3 * (4 * x2 - 21 * x - 9) * H0 + 128 * (x + 1) * H0_2
                 ) * tildeH01
...
//==========================================================================================//
//  nf-independent part of the mu-independent part of the unrenormalized
//  matching condition Qq at O(as^3). approximation from Eq. (3.53) of
//  [arXiv:1205.5727] used just as benchmark for the plots of the paper.
//------------------------------------------------------------------------------------------//
...
array<double, 3> MatchingCondition::a_Qq_PS_30_klmv(double x) const {
    ...
    double lower =
        (1. - x
         ) * (126.3546 * L1 * L1 + 353.8539 * L1 + 6787.608 * x + 3780.192 / x)
