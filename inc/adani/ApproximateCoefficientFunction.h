...
class ApproximateCoefficientFunction : public AbstractApproximate {
    public:
        ApproximateCoefficientFunction(
            const int &order, const char &kind, const char &channel,
            const double &damp_power = 1.0, const bool &NLL = true,
        ...
        double GetDampPower() const { return damp_power_; }
        void SetDampPower(const double &damp_power) { damp_power_ = damp_power; }
