...
int main(int argc, char** argv) {
    ...
    for(x=dx; x<1; x+=dx) {
        ...
        cout << x << "   " << gluonF2.Regular(x*(1+4*mQ))/ norm << "   " << quarkF2.Regular(x*(1+4*mQ))/ norm << "   " << gluonFL.Regular(x*(1+4*mQ))/ norm << "   " << quarkFL.Regular(x*(1+4*mQ))/ norm << "   " << C2_g21(x, mQ) << "   " <<C2_ps21(x, mQ)<< "   " << CL_g21(x, mQ) << "   " <<CL_ps21(x, mQ) << endl;
    }
