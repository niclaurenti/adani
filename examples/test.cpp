...
int main() {
    ...
    for(int i=0; i<N; i++) {
        for(int j=0; j<N; j++) {
            ...
            cout << "C2_g^(3)(x=" << x << ", Q="<< Q << ") = "<< F2g.fxBand(x, m2Q2, m2mu2, nf) << endl ;
            cout << "C2_ps^(3)(x=" << x << ", Q="<< Q << ") = "<< F2q.fxBand(x, m2Q2, m2mu2, nf) << endl ;
            cout << "CL_g^(3)(x=" << x << ", Q="<< Q << ") = "<< FLg.fxBand(x, m2Q2, m2mu2, nf) << endl ;
            cout << "CL_ps^(3)(x=" << x << ", Q="<< Q << ") = "<< FLq.fxBand(x, m2Q2, m2mu2, nf) << endl ;
        }
    }
    return 0;
}
